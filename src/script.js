...
function playGame(rounds) {
  ...
  function updateTimer() {
    ...
    const tenths = Math.floor((elapsedTime / 100) % 10);
    const seconds = Math.floor(elapsedTime / 1000);

    timerElement.textContent = `${formatTime(seconds)}:${formatTime(tenths)}:${formatTime(hundredths)}`;
  }
...
function displayTimes() {
  ...
  times.forEach((time, index) => {
    timesHTML += `<li>Round ${index + 1}: ${formatTime(time)}</li>`;
    totalTime += time;
  });

  timesHTML += `<li><strong>Total Time:</strong> ${formatTime(totalTime)}</li></ul>`;
