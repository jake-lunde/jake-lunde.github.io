...
// Set allWords to the words array
allWords = words.slice(); // Create a copy of the words array
...
function playGame(rounds) {
  ...
  function startRound() {
    ...
    let currentDifficulty; // Declare currentDifficulty here

    // Calculate difficulty range for this round
    const minDifficulty = Math.max(0, 40 - (round - 1) * 30);
    const maxDifficulty = Math.min(40 - round * 30, 40);

    // Generate word1 and word2 based on difficulty range
    word1 = getRandomWord();
    word2 = getRandomWord();

    while (
      (currentDifficulty < minDifficulty || currentDifficulty >= maxDifficulty) ||
      currentDifficulty <= 0
    ) {
      if (word1 > word2) {
        [word1, word2] = [word2, word1];
      ...
      if (iterations >= maxIterations) {
        console.log("Exceeded maximum iterations. Unable to find suitable words.");
        return;
      }
    }
  ...
  function validateWord() {
    ...
    if (userInput < word1 || userInput > word2 || userInput.length !== WORD_LENGTH) {
      console.log("Failure! The word is not alphabetically between the two words or not 5 letters long.");
      ...
      if (!isAlphabeticallyBetween(userInput, word1, word2)) {
        ...
        }, 100); // Delay of 1 second before progressing to the next round
      }
    }
  }
}
